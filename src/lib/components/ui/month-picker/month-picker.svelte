...
<PopoverPrimitive.Root bind:open {onOpenChange}>
	<PopoverPrimitive.Trigger
		...
		class={cn(
			'border-input dark:bg-input/30 dark:hover:bg-input/50 focus-visible:border-ring focus-visible:ring-ring/50 flex h-9 w-full items-center justify-between rounded-md border bg-transparent px-2.5 text-left text-sm shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-3 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50',
	...
	<PopoverPrimitive.Portal>
		<PopoverPrimitive.Content
			...
			sideOffset={8}
			class="bg-popover text-popover-foreground z-50 w-(--bits-popover-anchor-width) rounded-md p-2.5 shadow-md ring-1 ring-foreground/10 outline-none data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95"
			{onOpenAutoFocus}
		>
